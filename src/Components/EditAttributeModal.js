...
const EditAttributeModal = ({ isOpen, onClose, rowData, onUpdateAttribute }) => {
  const [name, setName] = useState("");
  const [value, setValue] = useState("");

  // Set input fields with row data when the modal is opened
  useEffect(() => {
    if (rowData) {
  ...
  const handleSubmit = () => {
    onUpdateAttribute({ id: rowData.id, name, value }); // Pass the updated data
  ...
  return (
    <div className="fixed inset-0 bg-[rgba(0,0,0,0.5)] flex justify-center items-center z-50">
      <div className="bg-white w-[500px] rounded-lg shadow-lg p-6 relative cursor-pointer">
        ...
        <h2 className="text-lg font-semibold text-gray-700 mb-4">Edit User Attribute</h2>
